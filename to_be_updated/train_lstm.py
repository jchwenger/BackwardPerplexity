...
if __name__=='__main__':
    ...
    motherDataset = TokenTextBOS(h5_file=destination_path, attn_length=model_params['desired_attn_length'], backwards=backwards)
    ...
    if(not os.path.exists(indices_path)):
        ...
        print(f'Indices {indices_path} do not exist, creating them...')
    ...
    trainer = MinGRU_Trainer(model=model,optim=optim,scheduler=scheduler,
    ...
    trainer.train_steps(steps=steps_to_train,save_every=2000,aggregate=aggregate,
                        ...
                        batch_size=batch_size,valid_every=valid_every,resume_batches=True,pickup=True)